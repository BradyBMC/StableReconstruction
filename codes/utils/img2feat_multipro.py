...
def thread_start(parser, tid, start, end, chunk):
    # Set Parameters
...
def main():
    ...
    thread_count = 2
    thread = [None for x in range(thread_count)]

    for tid in range(thread_count):
        chunk = int((imgidx[1] - imgidx[0]) / thread_count)
        start = chunk * tid + imgidx[0]
        end = chunk * (tid + 1) + imgidx[0]
        # thread[tid] = Process(target=thread_start, args=(tid, start, end, chunk)) 
        thread[tid] = threading.Thread(target=thread_start, args=(parser, tid, start, end, chunk)) 
